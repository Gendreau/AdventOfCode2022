...
public class FourteenOne {
	...
	public static int normalizationConstant=0;
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		while (finishFlag == false) {
			caveMap = dropSand(caveMap, dropLocation-normalizationConstant, 3);
			counter++;
		}
	...
	public static char[][] parseText(Scanner sc) {
		...
		normalizationConstant = Collections.min(rockXs);
		for (int i=0; i<rockXs.size(); i++) {
			rockXs.set(i, rockXs.get(i)-normalizationConstant);
		}
		
		char[][] caveMap = new char[Collections.max(rockYs)+4][Collections.max(rockXs)+1];
		...
		caveMap[0][dropLocation-normalizationConstant] = '|';
		caveMap[1][dropLocation-normalizationConstant] = '|';
		caveMap[2][dropLocation-normalizationConstant] = 'V';
		caveMap[3][dropLocation-normalizationConstant] = '+';
